# Modified by Neurwerk, 2025-2026: apply the SSO account-service runtime patch.
...
def create_flask_app_with_configs() -> DifyApp:
    ...
    """
    # Keycloak SSO support: add bypass_register_check to
    # AccountService.create_account / RegisterService.register (see
    # services/account_service_patch.py). Must run before any blueprint import
    # that calls RegisterService.register.
    from services import account_service_patch

    account_service_patch.apply()
...
def _auto_setup(app: DifyApp) -> None:
    """Create the admin account + workspace on first boot if AUTO_SETUP_ADMIN_EMAIL is set."""
    ...
    with app.app_context():
        ...
        setup_done = db.session.scalar(select(DifySetup).limit(1))
        if setup_done:
            return
        ...
        admin_email = dify_config.AUTO_SETUP_ADMIN_EMAIL
        admin_password = dify_config.AUTO_SETUP_ADMIN_PASSWORD
        if not admin_email or not admin_password:
            logger.info("AUTO_SETUP_ADMIN_EMAIL not set — skipping auto-setup")
            return

        ws_name = dify_config.DEFAULT_WORKSPACE_NAME or admin_email.split("@")[0]
        logger.info("Auto-setup: creating admin account %s with workspace '%s'", admin_email, ws_name)
        ...
        if tenant:
            logger.info(
                "Auto-setup complete. Workspace ID: %s. "
                "ENFORCE_SINGLE_WORKSPACE is %s — new SSO users will share this workspace.",
                tenant.id,
                "enabled" if dify_config.ENFORCE_SINGLE_WORKSPACE else "disabled",
            )
