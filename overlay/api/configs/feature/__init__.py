# Modified by Neurwerk, 2025-2026: add Keycloak OIDC feature configuration.
...
class AuthConfig(BaseSettings):
    ...
    KEYCLOAK_OIDC_ISSUER_URL: str | None = Field(
        description="Keycloak OIDC issuer URL, e.g. https://auth.example.com/realms/myrealm",
        default=None,
    )

    KEYCLOAK_OIDC_CLIENT_ID: str | None = Field(
        description="Keycloak OIDC client ID",
        default=None,
    )

    KEYCLOAK_OIDC_CLIENT_SECRET: str | None = Field(
        description="Keycloak OIDC client secret",
        default=None,
    )

    KEYCLOAK_OIDC_SCOPES: str = Field(
        description="OIDC scopes for Keycloak authentication",
        default="openid email profile",
    )

    KEYCLOAK_OIDC_STATE_MAX_AGE_SECONDS: int = Field(
        description="Maximum lifetime of a browser-bound Keycloak OAuth state value",
        default=300,
    )

    ENFORCE_SINGLE_WORKSPACE: bool = Field(
        description=(
            "When enabled, all SSO/OAuth users are added to the first workspace "
            "instead of creating a personal workspace for each user. "
            "Set to true to enforce single-workspace mode."
        ),
        default=False,
    )

    ALLOW_SSO_REGISTER: bool = Field(
        description=(
            "Allow new account registration via SSO/OAuth even when ALLOW_REGISTER is false. "
            "Set to true to enable SSO-based registration independently of manual signup."
        ),
        default=True,
    )
...
class LoginConfig(BaseSettings):
    ...
    AUTO_SETUP_ADMIN_EMAIL: str | None = Field(
        description=(
            "Admin email for first-boot auto-setup. When set and no setup exists, "
            "creates the admin account and workspace automatically on startup."
        ),
        default=None,
    )

    AUTO_SETUP_ADMIN_PASSWORD: str | None = Field(
        description="Admin password for first-boot auto-setup.",
        default=None,
    )

    DEFAULT_WORKSPACE_NAME: str = Field(
        description=(
            "Workspace name used during first-boot auto-setup. "
            "If empty, defaults to the admin email prefix."
        ),
        default="",
    )
